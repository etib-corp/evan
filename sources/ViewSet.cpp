...
void evan::ViewSet::setView(std::size_t index, const utility::graphic::ViewF &view)
{
...
void evan::ViewSet::setSwapchainIndex(std::size_t index, std::size_t swapchainIndex)
{
    if (index >= _views.size()) {
        _views.resize(index + 1);
    }
