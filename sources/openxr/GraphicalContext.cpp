...
void evan::openxr::GraphicalContext::initializeSession()
{
	...
	std::shared_ptr<VulkanContext> vulkanContext =
		_renderingContext->getVulkanContext();
...
void evan::openxr::GraphicalContext::createVisualizedSpace()
{
	...
	XrSpace space;
	if (xrCreateReferenceSpace(_XRsession, &spaceCreateInfo, &space)
		!= XR_SUCCESS) {
...
void evan::openxr::GraphicalContext::createInstance()
{
	...
	PFN_xrGetVulkanGraphicsRequirements2KHR
		xrGetVulkanGraphicsRequirements2KHR					= nullptr;
	...
	if (xrGetInstanceProcAddr(_XRinstance,
		...
		!= XR_SUCCESS) {
		std::cerr
			<< "Failed to get xrGetVulkanGraphicsRequirements2KHR function"
			<< std::endl;
		return;
	}
	if (xrGetVulkanGraphicsRequirements2KHR(_XRinstance, _XRsystemID,
	...
	if (xrGetInstanceProcAddr(
		...
		!= XR_SUCCESS) {
		std::cerr << "Failed to get xrCreateVulkanInstanceKHR function"
				  << std::endl;
		return;
	}
	...
	if (xrCreateVulkanInstanceKHR(_XRinstance, &vkInstanceCreateInfo,
								  &_instance, &result)
		!= XR_SUCCESS) {
...
std::vector<std::string> evan::openxr::GraphicalContext::getInstanceExtensions()
{
