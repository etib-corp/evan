	...
	const std::vector<int64_t> &swapchainFormats)
{
	...
	if (swapchainFormatIt == swapchainFormats.end()) {
		std::cerr
			<< "SwapchainContext: No preferred swapchain format found, using "
...
void evan::openxr::SwapchainContext::init()
{
	if (_instance == XR_NULL_HANDLE || _session == XR_NULL_HANDLE) {
		std::cerr << "SwapchainContext: Invalid instance or session"
				  << std::endl;
		return;
	}
	...
	for (const auto &viewConfig: _viewsConfigurations) {
		...
		XrResult result = xrCreateSwapchain(_session, &swapchainCreateInfo,
											&swapchain.swapchain);
		...
		XrSwapchainImageBaseHeader *image =
			swapchainImage->getFirstImagePointer();

		xrEnumerateSwapchainImages(swapchain.swapchain,
								   swapchainImage->_swapchainImages.size(),
								   nullptr, image);
	}
}
...
XrSwapchainImageBaseHeader *evan::openxr::SwapChainImage::getFirstImagePointer()
{
...
void evan::openxr::SwapchainContext::createGraphicsPipeline()
{
	...
	pipelineLayoutInfo.setLayoutCount =
		0;	  // No descriptor set layout for now
	pipelineLayoutInfo.pushConstantRangeCount =
		0;	  // No push constants for now
