...
public:
   ...
   * @brief Holds Vulkan objects and properties required for texture image
   * creation.
   *
   * This structure encapsulates the necessary Vulkan handles and settings
   * needed to create and manage texture images, including the physical and
   * logical devices, command pool, MSAA sample count, and graphics queue.
   *
   */
  struct TextureImageCreationProperties {
    /*
   ...
   *
   * @note This function should be called when the swapchain needs to be
   * recreated, such as when the window is resized or the surface format
   * changes.
   */
...
protected:
   ...
   *
   * @note This function initializes the image views for each image in the
   * swapchain, which are used for rendering operations.
   *
   * @see createImageView
   */
  ...
  std::map<std::string, VkImageView>
      _textureImageView; // Image views for texture images mapped by their names
  std::map<std::string, VkSampler>
      _textureSampler; // Samplers for texture images mapped by their names
   ...
   * view.
   *
   * @note This function initializes the `_textureImageView` member by creating
   * an image view for each texture image in the `_textureImage` map using the
   * specified logical device. The image views are created with the
   * `VK_FORMAT_R8G8B8A8_SRGB` format and the color aspect flag.
   */
   ...
   * using the provided logical device and physical device. The sampler is
   * created with the given sampler creation info or defaults to standard
   * settings if none are provided.
   *
   * @param logicalDevice The Vulkan logical device used to create the sampler.
   * @param physicalDevice The Vulkan physical device used to query properties
   * for sampler creation.
   * @param textureName The name of the texture for which the sampler is being
   * created.
   * @param samplerInfo Optional sampler creation info; if not provided, default
   * settings are used.
   */
   ...
   * framebuffers, and graphics pipeline using the provided properties. It
   * prepares the swapchain context for rendering operations.
   *
   * @param surface The Vulkan surface to present images to.
   * @param physicalDevice The Vulkan physical device (GPU) to use.
   * @param logicalDevice The Vulkan logical device associated with the physical
   * device.
   ...
   *
   * @note This function should be called before destroying the swapchain
   * context.
   */
   ...
   *
   * This function initializes a framebuffer for each image view in the
   * swapchain. Each framebuffer is configured with a color attachment, a depth
   * attachment, and the corresponding swapchain image view. The framebuffers
   * are stored in the `_swapchainFramebuffers` member variable.
   *
  ...
  VkDescriptorPool
      _descriptorPool; // Vulkan descriptor pool for managing descriptor sets
  std::vector<VkDescriptorSet>
      _descriptorSets; // Vector of Vulkan descriptor sets for uniform data
  VkDescriptorSetLayout
      _descriptorSetLayout; // Vulkan descriptor set layout for uniform data
  VkDebugUtilsMessengerEXT
      _debugMessenger; // Vulkan debug messenger for validation layers
   ...
   *
   * This function defines the layout of descriptor sets used in the graphics
   * pipeline, specifying the types and counts of resources (uniform buffers,
   * combined image samplers) that will be bound to the shaders.
   *
   * @param logicalDevice The Vulkan logical device used to create the
   * descriptor set layout.
   *
   * @throws std::runtime_error If the Vulkan descriptor set layout creation
   * fails.
   */
   ...
   * member variable.
   *
   * @param logicalDevice The Vulkan logical device used to allocate descriptor
   * sets.
   * @param imagesViewsAndSamplers A map of image views and their corresponding
   * samplers to be bound.
   *
   * @throws std::runtime_error If the Vulkan descriptor set allocation or
   * update fails.
   *
  ...
  std::vector<VkDeviceMemory>
      _uniformBuffersMemory; // Vector of Vulkan memory for uniform buffers
  std::vector<void *>
      _uniformBuffersMapped; // Vector of mapped pointers to uniform buffer data
   ...
   *
   * @param logicalDevice The Vulkan logical device used to create the uniform
   * buffers.
   * @param physicalDevice The Vulkan physical device used to query memory
   * properties.
   *
...
private:
  SwapchainContextCreationProperties
      _creationProperties; // Properties used for creating the swapchain context
   ...
   * from the list of available formats. It prioritizes the
   * VK_FORMAT_B8G8R8A8_SRGB format with VK_COLOR_SPACE_SRGB_NONLINEAR_KHR color
   * space, but will fall back to the first available format if the preferred
   * one is not found.
   *
   ...
   * VK_PRESENT_MODE_MAILBOX_KHR mode for its low latency and ability to avoid
   * tearing, but will fall back to VK_PRESENT_MODE_FIFO_KHR if the preferred
   * mode is not found.
   *
   ...
   * capabilities.
   *
   * @param capabilities The VkSurfaceCapabilitiesKHR structure containing
   * surface capabilities.
   * @param window A pointer to the GLFW window used to determine the swap
   * extent.
   *
   * @return VkExtent2D The chosen swap extent (width and height) for the
   * swapchain images.
   */
  ...
  /**
   * @brief Creates color resources for the swapchain, including a color image
   * and its associated image view.
   *
   * This function initializes a color image with the specified format, extent,
   * and sample count, and allocates memory for it. Additionally, it creates an
   * image view for the color image to be used in rendering operations.
   *
   * @param logicalDevice The Vulkan logical device used to create the image and
   * image view.
   * @param physicalDevice The Vulkan physical device used to allocate memory
   * for the image.
   * @param msaaSamples The number of samples per pixel for multisampling
   * (MSAA).
   *
  ...
  /**
   * @brief Creates depth resources for the swapchain, including a depth image
   * and its associated image view.
   *
   * This function initializes a depth image with the appropriate format,
   * extent, and sample count, allocates memory for it, and creates an image
   * view for the depth image to be used in rendering operations.
   *
   * @param properties The properties required for texture image creation,
   * including physical and logical devices, command pool, MSAA sample count,
   * and graphics queue.
   *
   * @throws std::runtime_error If image or image view creation fails.
   */
  ...
  /**
   * @brief Provides default sampler creation info based on physical device
   * properties.
   *
   * This function generates a VkSamplerCreateInfo structure with default
   * settings suitable for most texture sampling scenarios. It uses the provided
   * physical device properties to set the maximum anisotropy level, ensuring
   * optimal texture quality. The sampler is configured for linear filtering,
   * repeat addressing mode, and standard mipmap settings.
   *
   * @param properties The VkPhysicalDeviceProperties structure containing
   * properties of the physical device.
   *
   * @return VkSamplerCreateInfo A structure containing default sampler creation
   * settings.
   */
  VkSamplerCreateInfo
  ...
  /**
   * @brief Creates and allocates a single descriptor set for the swapchain
   * context.
   *
   * This function allocates a descriptor set from the descriptor pool and
   * configures it to bind a uniform buffer and a texture sampler. The
   * descriptor set is added to the `_descriptorSets` member variable.
   *
   * @param logicalDevice The Vulkan logical device used to allocate the
   * descriptor set.
   * @param textureImageView The Vulkan image view representing the texture to
   * be sampled.
