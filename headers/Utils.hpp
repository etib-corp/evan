...
public:
  ...
  struct SwapChainSupportDetails {
    ...
    /*
     * @brief A list of supported presentation modes (e.g., FIFO, Mailbox,
     * etc.).
     */
  ...
  struct QueueFamilyIndices {
     ...
     * indicating that the required queue families are available. False
     * otherwise.
     */
    bool isComplete() {
   ...
   * @brief Encapsulates the properties required to create a Vulkan image and
   * allocate its memory.
   *
   * This structure holds all necessary parameters for creating a VkImage object
   * and allocating its associated device memory in Vulkan. It includes device
   * handles, image dimensions, format, usage flags, and references to the
   * resulting image and memory objects.
   *
   */
  struct CreateImageProperties {
    /*
     * @brief The Vulkan logical device used for image creation and memory
     * allocation.
     */
    ...
    VkImage &_image;
    /*
     * @brief A reference to the VkDeviceMemory object that will be allocated
     * for the image.
     */
   ...
   * @brief Encapsulates the properties required to perform an image layout
   * transition in Vulkan.
   *
   * This structure holds all necessary Vulkan objects and parameters needed to
   * transition the layout of a VkImage, such as device, command pool, queue,
  ...
  struct TransitionImageLayoutProperties {
    ...
    VkDevice _logicalDevice;
    /*
     * @brief The Vulkan command pool used to allocate command buffers for the
     * layout transition.
     */
    VkCommandPool _commandPool;
    /*
     * @brief The Vulkan graphics queue used to submit the command buffer for
     * execution.
     */
  ...
  static void
  transitionImageLayout(const TransitionImageLayoutProperties &properties);

  static bool isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface,
                               std::vector<const char *> deviceExtensions);

  /**
   * @brief Properties required for creating a Vulkan buffer and allocating its
   * memory.
   *
  ...
  struct CreateBufferProperties {
    /*
     * @brief The Vulkan logical device used for buffer creation and memory
     * allocation.
     */
    VkDevice _logicalDevice;
    /*
  ...
  struct GenerateMipmapsProperties {
    ...
    VkDevice _logicalDevice;
    /*
     * @brief The Vulkan command pool used to allocate command buffers for
     * mipmap generation.
     */
    VkCommandPool _commandPool;
    /*
   ...
   * @brief Encapsulates properties required for copying data between Vulkan
   * buffers.
   *
   * This structure holds all necessary Vulkan objects and parameters needed to
   * perform a buffer-to-buffer copy operation, such as the logical device,
   * command pool, graphics queue, source and destination buffers, and the size
   * of the data to copy.
   *
   */
  struct CopyBufferProperties {
  ...
  getMaxUsableSampleCount(const VkPhysicalDevice &physicalDevice);
...
private:
  ...
  static VkCommandBuffer beginSingleTimeCommands(VkDevice logicalDevice,
                                                 VkCommandPool commandPool);
  static void endSingleTimeCommands(VkDevice logicalDevice,
                                    VkCommandPool commandPool,
