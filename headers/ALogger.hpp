 ...
 * method to be implemented by derived classes, along with member variables for
 * environment and program name.
 */
class ALogger {
public:
  /**
   ...
   * @var ERROR Represents an error that may not cause termination but indicates
   * a significant issue.
   * @var WARNING Represents a warning that indicates a potential problem but
   * does not require immediate action.
   * @var INFO Represents informational messages that provide general
   * information about the program's execution.
   * @var DEBUG Represents debug messages that provide detailed information for
   * debugging purposes.
   */
  enum LogLevel {
    FATAL, ///< Fatal error that causes the program to terminate
    ERROR, ///< Error that may not cause termination but indicates a significant
           ///< issue
    WARNING, ///< Warning that indicates a potential problem but does not
             ///< require immediate action
    INFO, ///< Informational messages that provide general information about the
          ///< program's execution
    DEBUG ///< Debug messages that provide detailed information for debugging
          ///< purposes
  };
...
protected:
   ...
   * the origin of the log entry.
   * @param message The message to log.
   * @param logLevel The log level (e.g., ERROR, WARNING, INFO, DEBUG).
   * @param caller The name of the function or method that called this function.
   */
  virtual void log(const std::string &message, const std::string &logLevel,
                   const std::string &caller) const = 0;

  std::string _env; ///< The environment in which the logger operates (e.g., DEV
                    ///< or PROD)
